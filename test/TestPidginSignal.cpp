...
static void
account_status_changed(PurpleAccount *account, PurpleStatus *old, PurpleStatus *newStatus,
						gpointer data)
{
	purple_debug_misc("signals test", "account-status-changed (%s, %s, %s)\n",
					purple_account_get_username(account),
					purple_status_get_name(old),
					purple_status_get_name(newStatus));
}
...
gboolean
test_signals_unload(PurplePlugin *plugin)
{
	void *jabber_handle = purple_plugins_find_with_id("prpl-jabber");
